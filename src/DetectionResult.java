...
public final class DetectionResult {
    ...
    public BoundingBox 2D getBox2d() { return box2d; }
    public BoundingBox 3D getBox3d() { return box3d; } // always null
    ...
    public static final class Builder {
        ...
        private float            futionWeight = 1.0f; 
        ...
        public Builder box3d(BoundingBox3D b) {
            this.box3d     = b;
            return this;
        }
    ...
    public record BoundingBox2D(int x, int y, int width, int height) {
        ...
        /**
         * Filters a list to only hazzardous, high confidence detections.
         */
        public static List<DetectionResult> filter Hazards(List<DetectionResult> results, float minConf) {
            return results.stream().filter(r -> r.terrainClass.isHazardous() && r.confidence >= minConf).sorted((a, b) -> Float.compare(b.confidence, a.confidence)).toList();
        }

        /**
         * Returns the closest detection by 30 centroid distance. 
         */
        public static DetectionResult closes(List <DetectionResult> results ) {
            return results.stream().filter(DetectionResult::has3dBox).min((a,b ) -> Double.compare(a.box3d.distanceToOrigin(), b.box3d.distanceToOrigin())).orElseThrow(() -> new IllegalArgumentException("No 3D detections available"));
        }
